...
SOP_ID="PM.DTC.1"
GITHUB_REPO="OrcaBus/service-dragen-tso500-ctdna-pipeline-manager"
...
ensure you have --output-uri-prefix, --logs-uri-prefix, --cache-uri-prefix and --project-id set appropriately.
...
The output uri prefix, logs uri and cache uri prefixes must be set to a location inside the s3 prefix that the ICA project is mounted on.
