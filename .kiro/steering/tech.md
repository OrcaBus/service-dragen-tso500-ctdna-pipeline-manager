...
- **AWS CDK v2** (`aws-cdk-lib ^2.258.1`) with TypeScript
- **`@orcabus/platform-cdk-constructs`** — internal shared constructs library (pinned to `1.7.0`)
...
- **Amazon EventBridge** — event bus `OrcaBusMain`, source `orcabus.dragentso500ctdna`
- **AWS SSM Parameter Store** — configuration under `/orcabus/workflows/dragen-tso500-ctdna/`
- **AWS Schemas Registry** — JSON schema validation for draft events
- **Amazon ECS** — ECS task for tabix operations
